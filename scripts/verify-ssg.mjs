...
const ROUTES = [
  ...
  },
  {
    ...
    custom: (body, head) => [
      ...
      ['bgImage hero = /images/services/junk-removal.webp', body.includes('/images/services/junk-removal.webp')],
      ['Mobile hero variant wired (picture/source)',
        body.includes('/images/services/junk-removal-mobile.webp')],
      ['7 page-content imgs (junk-removal/*)',
        (body.match(/<img[^>]*src="\/images\/junk-removal\/[a-z-]+\.webp"/g) || []).length === 7],
      ['ReviewWidget island in <section id="reviews">',
        body.includes('<section id="reviews"') && body.includes('<astro-island') && body.includes('ReviewWidget')],
      ['"New Service Request" button + modal trigger script',
        body.includes('id="junk-removal-modal-trigger"') && body.includes("CustomEvent('open-service-modal')")],
    ],
  },
  {
    path: '/dumpster-rental-service-hubbard',
    file: 'dist/dumpster-rental-service-hubbard/index.html',
    title: 'Dump Trailer Rental Service Hubbard TX | Rankin Waste Management',
    description: 'Dump trailer rental in Hubbard, TX — 14-yard dump trailers. Same-day delivery, 3-day standard rentals, flat-rate pricing. Call (254) 205-6125.',
    ...
    custom: (body, head) => [
      ['H1 = "Dump Trailer Rental Service in Hubbard"',
        /<h1[^>]*>\s*Dump Trailer Rental Service in Hubbard\s*<\/h1>/.test(body)],
      ['Service serviceType "Dump Trailer Rental"', head.includes('"serviceType":"Dump Trailer Rental"')],
      ['Service @id present', head.includes('"@id":"https://rankinwaste.com/dumpster-rental-service-hubbard/#service"')],
      ['Service has 2 OfferCatalog items',
        (head.match(/"@type":"Offer","itemOffered":/g) || []).length === 2],
      ['Service aggregateRating reviewCount=284 (under provider)', head.includes('"reviewCount":"284"')],
      ['Service review array has 3 reviews', (head.match(/"@type":"Review","author":/g) || []).length === 3],
      ...
      ['2 page-content imgs (dumpster-rental/*)',
        (body.match(/\/images\/dumpster-rental\/[a-z-]+\.webp/g) || []).length >= 2],
      ['ReviewWidget island', body.includes('ReviewWidget') && body.includes('<astro-island')],
      ['"New Service Request" button + modal trigger script',
        body.includes('id="dumpster-rental-modal-trigger"') && body.includes("CustomEvent('open-service-modal')")],
    ],
  },
  {
    ...
    custom: (body, head) => [
      ...
      ['Service @id present', head.includes('"@id":"https://rankinwaste.com/garbage-collection-service-hubbard/#service"')],
      ['Service has 3 OfferCatalog items',
        (head.match(/"@type":"Offer","itemOffered":/g) || []).length === 3],
      ['Service aggregateRating reviewCount=284 (under provider)', head.includes('"reviewCount":"284"')],
      ['Service review array has 4 reviews', (head.match(/"@type":"Review","author":/g) || []).length === 4],
  ...
  },
  {
    ...
    custom: (body, head) => [
      ...
      ['Organization schema present', head.includes('"@type":"Organization"')],
